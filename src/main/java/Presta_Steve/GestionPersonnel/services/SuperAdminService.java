...
public class SuperAdminService implements ISuperAdminService,UserDetailsService {
    ...
    public void modifierPassword(String email, String password){
        Optional<Utilisateur> supTrouve =  this.superAdminRepository.findByEmailSup(email);
        if (supTrouve.isPresent()) {
            Utilisateur supChangePass = supTrouve.get();
            supChangePass.setMdpSup(this.PasswordEncoder.encode(password));
            this.superAdminRepository.save(supChangePass);
        }
