...
private final SuperAdminRepository superAdminRepository;
  ...
  public void sendNewCode(String email){
    Optional<Utilisateur> findSup = this.superAdminRepository.findByEmailSup(email);
    if (findSup.isEmpty()) {
      throw new RuntimeException("une erreur s'est produit lors de l'envoi du nouveau code, cela est lie a votre email");
    }
    Utilisateur supTrouve = findSup.get();

    ActivationCompte activationCompte = new ActivationCompte();
    activationCompte.setIdSuperAdmin(supTrouve.getIdSup());
    Instant creation = Instant.now();
    activationCompte.setCreationDate(creation);
    Instant expiration = creation.plus(10,MINUTES); 
    activationCompte.setExpirationDate(expiration);
